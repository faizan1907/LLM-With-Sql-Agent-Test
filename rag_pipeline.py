    ...
    try:
        ...
        **=== OTHER QUERY RULES ===**
        ...
        - **MANDATORY RULE: NO NULL VALUES IN OUTPUT OR KEY CRITERIA**
            - **Strict `IS NOT NULL` Enforcement**: For EVERY column expression that will appear in the final `SELECT` list of the query, you MUST add a `WHERE` clause condition ensuring that the result of that expression `IS NOT NULL`.
            - Additionally, for any column used in another `WHERE` clause condition (beyond the `IS NOT NULL` checks themselves) or in an `ORDER BY` clause, these columns (or the expressions they are part of) MUST also have an `IS NOT NULL` condition applied in the `WHERE` clause targeting their final/effective value.
            ...
            - **Example**: If the query intends to select `col_a` and `col_b`, and filter by `col_c = 'some_value'`, it MUST be transformed to effectively include: `SELECT col_a, col_b FROM my_table WHERE col_a IS NOT NULL AND col_b IS NOT NULL AND col_c IS NOT NULL AND col_c = 'some_value';` (assuming `col_c` itself isn't directly selected, its non-NULL status for filtering is also vital).
            - **Impact on JOINs (Especially LEFT JOINs)**: If a `LEFT JOIN` is used, and columns/expressions selected from the right-joined table evaluate to `NULL` for some rows, those rows WILL BE EXCLUDED due to this rule. This is an intentional consequence and makes the query stricter.
            - **No Exceptions (Unless Explicitly Overridden by Task)**: This `IS NOT NULL` enforcement is mandatory and must be applied universally UNLESS the original `Task Description` explicitly and specifically states "include NULL values for column_name" or "allow NULLs for column_name". In the absence of such an explicit override, all selected/referenced column expressions must result in non-NULL values.
        ...
        - **MANDATORY RULE: TRANSFORM NEGATIVE PERCENTAGE VALUES**
            - **Applicability**: This rule applies to columns identified as representing percentage values which are part of the final `SELECT` list.
            - **Identification of Percentage Columns**:
                - Examine column names for common indicators such as 'percent', 'pct', '%', 'rate', 'ratio', 'proportion', 'margin' (especially if the context or data type suggests a percentage).
                - Refer to the `Task Description` and `Required Data Summary` for explicit mentions or clear implications that a column represents a percentage.
                - If a column is the result of a calculation that definitionally yields a percentage (e.g., `(numerator_column / denominator_column) * 100.0`), it MUST be treated as a percentage column for this rule.
            - **Transformation Logic**:
                - If an identified percentage column (that is not NULL) contains a negative value (i.e., less than 0), that negative value MUST be replaced with the positive decimal value `0.00001` in the final query output.
                - If the percentage value is zero or positive, it should be returned as is (subject to other rules like NULL checks).
            - **SQL Implementation**: This transformation MUST be implemented directly in the `SELECT` clause using a `CASE` statement for each identified percentage column.
                - Example: `SELECT ..., CASE WHEN original_percentage_column < 0 THEN 0.00001 ELSE original_percentage_column END AS aliased_percentage_column_name, ... FROM ...`
                - The alias (`aliased_percentage_column_name`) should ideally be the original column name or the name expected in the output.
            - **Mandatory Application**: This transformation is mandatory for all identified and selected percentage columns. It must be applied UNLESS the `Task Description` explicitly states "allow negative percentages for column_name" or "do not transform negative percentages for column_name". In the absence of such an override, all negative percentages in selected columns must be transformed to `0.00001`.
            - **Interaction with NULLs**: This rule primarily addresses the transformation of non-NULL negative percentage values. The "MANDATORY RULE: NO NULL VALUES..." should separately ensure that the original percentage column (before this CASE statement) or the result of this CASE statement is not NULL in the final output, as per its own conditions. If `original_percentage_column` is `NULL`, the `CASE` expression will likely also yield `NULL` (unless `IS NULL` is explicitly handled within it, which is not the primary goal of *this* rule), and then the "NO NULL VALUES" rule would apply to that resulting `NULL`.
